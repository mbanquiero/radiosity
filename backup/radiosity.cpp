...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	*/
    ...
    // Creo e inicializo la malla que representa el spotlight (solo cosmetica, para ver
	// donde esta la luz. OJO que luz es un patch como cualquier otro, la unica diferencia
	// es que es el patch inicial. NO ES UN SPOT LIGHT, solo lo dibujo asi. y ojo que 
	// si bien tiene una direccion, (que en gral es la normal al patch), en teoria el angulo 
	// solido es todo el semi-hemisferio (fov = 180grados) 
                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    *(LPDWORD)lr.pBits = D3DCOLOR_RGBA( 128, 128, 128, 255 );
    ...
    V_RETURN( g_pEffect->SetFloat( "g_fCosTheta", cosf( g_fLightFov/2.0f ) ) );

	//--------------------------------------------------------------------------------------
    // Creo el shadowmap. 
	// lo voy a usar para calcular el factor de visibilidad de la ecuacion del radiosity
	// esto es H(i,j) = 1 si el patch j es visible desde el patch, 0 caso contrario. 
	...
	V_RETURN( pd3dDevice->CreateTexture( MAP_SIZE, MAP_SIZE,
		...
		g_pEffect->SetTexture( "g_RMap", g_pRMap);
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        for( int obj = 0; obj < NUM_OBJ; ++obj )
        {
            ...
            for( UINT p = 0; p < cPass; ++p )
            {
                ...
                {
					/* sin soporte de material por ahora. solo voy a usar la textura
...
void ProcesarVertices( IDirect3DDevice9* pd3dDevice,const D3DXMATRIX *pmView, const D3DXMATRIX *pmProj )
{
	...
	/*
    {
        ...
        D3DXVECTOR4 v4 = D3DXVECTOR4(v,1);
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	//g_LightView = *g_LCamera.GetViewMatrix();
	// Primero genero el shadow map
	...
	if(L==0 && cant_luces>1)    
		...
		g_pEffect->SetFloat( "g_mIL", 1.0f/(float)cant_luces);

	// paso a la siguiente luz
	}

	// dibujo la escena desde el pto de vista de la camara
	pd3dDevice->SetRenderState( D3DRS_ZENABLE, TRUE);
